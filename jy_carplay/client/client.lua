    ...
    if Entity(veh).state then
        ...
        if currQueue ~= nil then
            ...
            if data ~= nil then
                ...
                if exports.xsound:soundExists("jay_carplay_"..data.vehStr) then
                    exports.xsound:Destroy("jay_carplay_"..data.vehStr)
                end
                local volume = 1.0
                ...
                exports.xsound:PlayUrl("jay_carplay_"..data.vehStr, data.link, volume, false, {
                    onPlayStart = function(event)
                        table.insert(spawnedSounds, NetToVeh(data.veh))
                        SendNUIMessage({
                            event = "playbackStarted",
                            link = data.link,
                            vol = exports.xsound:getVolume("jay_carplay_"..data.vehStr)
                        })

                        exports.xsound:setTimeStamp("jay_carplay_"..data.vehStr, math.floor(currTime))
                        if Entity(veh).state.isPaused then
                            exports.xsound:Pause("jay_carplay_"..data.vehStr)
                            SendNUIMessage({
                                event = "setPicPaused",
                            })
                        end
                        local totalDurr = exports.xsound:getMaxDuration("jay_carplay_"..data.vehStr)
                        CreateThread(function()
                            while true do
                                Wait(999)

                                if totalDurr == 0 then
                                    totalDurr = exports.xsound:getMaxDuration("jay_carplay_"..data.vehStr)
                                end

                                if not exports.xsound:soundExists("jay_carplay_"..data.vehStr) then
                                    break
                                end

                                local currTime = exports.xsound:getTimeStamp("jay_carplay_"..data.vehStr)
                                Entity(NetToVeh(data.veh)).state:set('currTime', currTime, true)

                                if currTime+1 == totalDurr then
                                    TriggerEvent("jay_carplay:playsound", {event = "nextSong", veh =data.veh, vehStr = data.vehStr})
                                    shouldBreak = true
                                end
...
RegisterNetEvent('jay_carplay:playsound')
AddEventHandler("jay_carplay:playsound", function(data)
    if data.event == "url" then
        ...
        if not data.shouldForce and exports.xsound:soundExists("jay_carplay_"..data.vehStr) then
    ...
    elseif data.event == "resume" then
        if not exports.xsound:soundExists("jay_carplay_"..data.vehStr) then return end

        exports.xsound:Resume("jay_carplay_"..data.vehStr)
    ...
    elseif data.event == "setVolume" then
        ...
        exports.xsound:setVolume("jay_carplay_"..data.vehStr, newVol)
	...
	elseif data.event == "selectTime" then
	    ...
	    exports.xsound:setTimeStamp("jay_carplay_"..data.vehStr, math.floor(data.newTime))
        Wait(50)
    elseif data.event == "restartSong" then
        if not exports.xsound:soundExists("jay_carplay_"..data.vehStr) then return end

        exports.xsound:setTimeStamp("jay_carplay_"..data.vehStr, math.floor(0))
    elseif data.event == "nextSong" then
        ...
        if dat ~= nil then
            ...
            TriggerServerEvent("jay_carplay:syncmusic", peds, data.veh, dat)
        else
            SendNUIMessage({
                event = "updateTime",
                time = {currentTime = 0, totalDuration = 0}
...
RegisterNUICallback('callback', function(data)
    if data.veh ~= nil then
        ...
        local peds = getPeds(NetToVeh(data.veh))
        TriggerServerEvent("jay_carplay:syncmusic", peds, data.veh, data)
    end
end)

function notify(msg, type, title, time)
    if GetResourceState('jay_notifications') == 'started' then
        exports["jay_notifications"]:ShowNotification(type, title, msg, time)
    else
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    for k,v in pairs(spawnedSounds) do
        ...
        if exports.xsound:soundExists("jay_carplay_"..v) then
            exports.xsound:Destroy("jay_carplay_"..v)
            print("Destorying")
        end
